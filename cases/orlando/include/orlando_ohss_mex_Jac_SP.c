...
/* Check for the right number and size of input arguments */
 if ( nrhs != 3 ) {
   mexErrMsgTxt("orlando_ohss_Jac_SP requires 3 input vectors: V(18), F(3), RCT(41)");
 }
 ...
 if ( ( mrows != 3 )||( mcols != 1 ) ) {
   mexPrintf("Second orlando_ohss_Jac_SP input argument is of size F(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Second orlando_ohss_Jac_SP input argument should be a column vector F(3,1)");
 }
 mrows =  mxGetM(prhs[2]); mcols = mxGetN(prhs[2]);
 if ( (  mrows != 41 )||( mcols != 1 ) ) {
   mexPrintf("Third orlando_ohss_Jac_SP input argument is of size RCT(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Third orlando_ohss_Jac_SP input argument should be a column vector RCT(41,1)");
 }
